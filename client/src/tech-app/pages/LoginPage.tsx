/** Technician PWA — Login screen (mock, no real auth)
 ...
 *  No splash screen — login carries the branding. */
...
export function LoginPage({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <div className="min-h-screen flex flex-col max-w-md mx-auto bg-[#0f1a2e]">
      {/* Brand lockup — uses canonical app logo */}
      ...
      {/* Login form — white card on dark background */}
      <div className="bg-white rounded-t-3xl px-8 pt-10 pb-12 shadow-2xl">
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-12 rounded-xl border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#22c55e]/40 focus:border-[#22c55e] transition-shadow"
              />
            </div>
          </div>
          ...
          <button
            onClick={onLogin}
            className="w-full h-12 rounded-xl bg-[#22c55e] text-white font-semibold text-sm hover:bg-[#1db350] active:scale-[0.98] transition-all shadow-lg shadow-[#22c55e]/25"
          >
            Sign In
          </button>
        </div>

        <p className="text-[11px] text-center text-slate-400 mt-8">UI prototype — no real authentication</p>
      </div>
    </div>
  );
}
