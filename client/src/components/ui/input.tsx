...
const Input = React.forwardRef<HTMLInputElement, React.ComponentProps<"input">>(
  ({ className, type, ...props }, ref) => {
    ...
    return (
      <input
        type={type}
        // 2026-05-03 Phase E: text size migrated from raw `text-body`
        // to the canonical `text-input` semantic role token. Pixel-
        // identical (text-input is an alias of text-body at 15px / 22px
        // line-height). Centralizes form-input typography under a
        // single named role.
        className={cn(
          "flex h-9 w-full rounded-md border border-border-strong bg-surface px-3 py-2 text-input text-text-primary ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-text-disabled focus-visible:outline-none focus-visible:border-brand focus-visible:shadow-[0_0_0_2px_rgba(118,176,84,0.25)] disabled:cursor-not-allowed disabled:bg-surface-subtle disabled:text-[#94A3B8] disabled:border-[#E2E8F0] disabled:opacity-100",
          className
        )}
        ref={ref}
        {...props}
