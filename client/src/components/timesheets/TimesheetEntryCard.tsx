 ...
 *                          Drive/On-site category chip, a nested edit
 *                          button, non-bill indicator, lock badge, and
 *                          a clock-out button when the entry is running.
 *
 * variant "general-flat" → entry in the general group. Whole row is a
 *                          single edit button; "General" text label
 *                          replaces the chip; no clock-out button.
 *
 * Internal helpers EntryLockBadge and EntryTimeRange are shared by both
 * variants and are the single source of truth for those patterns in the
 * day view. CompactTimeEntryCard (week view) is a separate component
 * with a different data model (aggregated totals, no time ranges).
 */
...
export interface TimesheetEntryCardProps {
  ...
  onClockOut: () => void;
  /**
   * job-row only: suppress category chip when the parent group header
...
// Shared lock badge — identical appearance in both variants.
function EntryLockBadge() {
  return (
    <span className="inline-flex items-center gap-1 text-helper font-medium text-amber-600">
...
export function TimesheetEntryCard({
  ...
  if (variant === "general-flat") {
    return (
      <button
        ...
        onClick={onEdit}
        className={cn(
          "w-full flex items-center gap-3 px-3 py-2 text-left text-row transition-colors hover:bg-slate-50",
      ...
      >
        <span className="text-row font-semibold text-foreground">General</span>
        <EntryTimeRange startAt={entry.startAt} endAt={entry.endAt} />
        {isLocked && <EntryLockBadge />}
        <span
          className={cn(
            "ml-auto shrink-0 font-mono text-row font-bold tabular-nums",
  ...
  return (
    <div
      className={cn(
        "group flex items-center gap-2 border-t border-slate-100 px-3 py-2 text-row transition-colors first:border-t-0 hover:bg-slate-50",
        isLocked && "opacity-75",
      )}
      data-testid={`day-entry-compact-${entry.id}`}
    ...
    >
      ...
      >
        {!hideTypeChip && (
          <span
            className={cn(
              "inline-flex shrink-0 items-center rounded-full border px-2 py-0.5 text-helper font-semibold",
              style.chip,
            )}
      ...
      <span
        className={cn(
          "shrink-0 font-mono text-row font-bold tabular-nums",
          isRunning && "animate-pulse text-emerald-600",
        )}
        data-testid={`day-entry-compact-duration-${entry.id}`}
      >
