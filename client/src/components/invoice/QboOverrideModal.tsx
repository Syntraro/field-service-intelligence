...
// FORM modal (acknowledgement checkbox + reason textarea + 10-char min
// validation), not a pure confirm — AlertDialog is the wrong primitive
// for hosting interactive form input. The amber warning chrome + the
// destructive "Proceed with Change" button preserve the consequence
// emphasis that signals the gravity of bypassing QBO sync.
import {
  ModalShell,
  ModalHeader,
  ModalTitle,
...
export function QboOverrideModal({
  ...
  return (
    ...
    // rule #5 (ModalShell stays width-neutral). The intercepted
    // handleOpenChange resets `acknowledged` + `reason` on close so the
    // next open transition starts from a clean state — matches the
    // prior behavior where `handleOpenChange` was wired into the
