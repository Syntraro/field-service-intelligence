...
//
// Selecting a member here sets `?member=<id>` on the page URL via
// `onSelect`. The selection then drives every right-pane workspace
// tab (Overview / Schedule / Compensation / Access). The user picks
// once; every tab operates on the same person.
//
// Data source: GET /api/team. The legacy tabs each had their own
// query (some used `/api/team/technicians`, others `/api/team`); for
// the unified workspace we always show the full member roster — the
// inner tabs themselves still gate which members they can edit (e.g.
// Compensation only shows technicians) and surface a graceful empty
// state when the selected member isn't valid for that tab.
