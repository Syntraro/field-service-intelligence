...
SET default_tax_rate_new = 
  CASE 
    WHEN default_tax_rate ~ '^\d+\.?\d*$' THEN default_tax_rate::NUMERIC(5,2)
    ELSE 13.00
  END;
...
UPDATE parts 
SET 
  cost_new = CASE WHEN cost ~ '^\d+\.?\d*$' THEN cost::NUMERIC(12,2) ELSE NULL END,
  markup_percent_new = CASE WHEN markup_percent ~ '^\d+\.?\d*$' THEN markup_percent::NUMERIC(5,2) ELSE NULL END,
  unit_price_new = CASE WHEN unit_price ~ '^\d+\.?\d*$' THEN unit_price::NUMERIC(12,2) ELSE NULL END;

-- Invoices: Convert all money fields
UPDATE invoices 
SET 
  subtotal_new = CASE WHEN subtotal ~ '^\d+\.?\d*$' THEN subtotal::NUMERIC(12,2) ELSE 0.00 END,
  tax_total_new = CASE WHEN tax_total ~ '^\d+\.?\d*$' THEN tax_total::NUMERIC(12,2) ELSE 0.00 END,
  total_new = CASE WHEN total ~ '^\d+\.?\d*$' THEN total::NUMERIC(12,2) ELSE 0.00 END,
  amount_paid_new = CASE WHEN amount_paid ~ '^\d+\.?\d*$' THEN amount_paid::NUMERIC(12,2) ELSE 0.00 END,
  balance_new = CASE WHEN balance ~ '^\d+\.?\d*$' THEN balance::NUMERIC(12,2) ELSE 0.00 END;

-- Invoice Lines: Convert all money fields
UPDATE invoice_lines 
SET 
  unit_cost_new = CASE WHEN unit_cost ~ '^\d+\.?\d*$' THEN unit_cost::NUMERIC(12,2) ELSE NULL END,
  unit_price_new = CASE WHEN unit_price ~ '^\d+\.?\d*$' THEN unit_price::NUMERIC(12,2) ELSE 0.00 END,
  tax_rate_new = CASE WHEN tax_rate ~ '^\d+\.?\d*$' THEN tax_rate::NUMERIC(5,4) ELSE 0.0000 END,
  line_subtotal_new = CASE WHEN line_subtotal ~ '^\d+\.?\d*$' THEN line_subtotal::NUMERIC(12,2) ELSE 0.00 END,
  tax_amount_new = CASE WHEN tax_amount ~ '^\d+\.?\d*$' THEN tax_amount::NUMERIC(12,2) ELSE 0.00 END,
  line_total_new = CASE WHEN line_total ~ '^\d+\.?\d*$' THEN line_total::NUMERIC(12,2) ELSE 0.00 END;

-- Payments: Convert amount
UPDATE payments 
SET amount_new = CASE WHEN amount ~ '^\d+\.?\d*$' THEN amount::NUMERIC(12,2) ELSE NULL END;
...
SET 
  labor_cost_per_hour_new = CASE WHEN labor_cost_per_hour ~ '^\d+\.?\d*$' THEN labor_cost_per_hour::NUMERIC(8,2) ELSE NULL END,
  billable_rate_per_hour_new = CASE WHEN billable_rate_per_hour ~ '^\d+\.?\d*$' THEN billable_rate_per_hour::NUMERIC(8,2) ELSE NULL END;
...
BEGIN
  IF EXISTS (
    ...
    SET unit_price_override_new = CASE WHEN unit_price_override ~ '^\d+\.?\d*$' THEN unit_price_override::NUMERIC(12,2) ELSE NULL END;
  END IF;
END $$;
