/**
 * Client CSV Import Routes (v1)
 *
 * POST /api/client-import/preview  — Parse, map, validate, return preview
 * POST /api/client-import/execute  — Execute validated import rows
 *
...
router.post(
  ...
  asyncHandler(async (req: AuthedRequest, res: Response) => {
    ...
    const companyCache = new Map<string, { exists: boolean; name: string }>();
    const validatedRows = [];
    ...
    for (const row of validatedRows) {
      ...
      if (row.normalized.companyName) {
        if (row.matchesExisting) {
          matchedExisting++;
        } else if (!seenNewCompanies.has(row.normalized.companyName)) {
          seenNewCompanies.add(row.normalized.companyName);
          newCompanies++;
        }
      }
    }
...
router.post(
  ...
  asyncHandler(async (req: AuthedRequest, res: Response) => {
    ...
    let locationsCreated = 0, contactsCreated = 0;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (result.success) {
        importedRows++;
        if (result.locationId) locationsCreated++;
